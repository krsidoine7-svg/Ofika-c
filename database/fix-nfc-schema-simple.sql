-- =====================================================
-- CORRECTION SIMPLE DES ERREURS DE SCHÉMA NFC
-- =====================================================
...
-- 3. Supprimer la table nfc_profiles si elle existe (pour la recréer avec le bon schéma)
DROP TABLE IF EXISTS nfc_profiles CASCADE;

-- 4. Créer la table nfc_profiles avec le bon schéma
...
-- 5. Activer RLS sur nfc_profiles
ALTER TABLE nfc_profiles ENABLE ROW LEVEL SECURITY;

-- 6. Créer les politiques RLS pour nfc_profiles
...
CREATE INDEX idx_nfc_profiles_user_id ON nfc_profiles(user_id);
CREATE INDEX idx_nfc_profiles_nfc_link ON nfc_profiles(nfc_link);
CREATE INDEX idx_nfc_profiles_custom_url ON nfc_profiles(custom_url);
CREATE INDEX idx_nfc_profiles_username ON nfc_profiles(username);
CREATE INDEX idx_nfc_profiles_status ON nfc_profiles(status);
...
-- 9. Vérifier que les colonnes ont été ajoutées
...
AND table_schema = 'public'
AND column_name IN ('bio', 'company', 'full_name', 'job_title', 'phone', 'email', 'user_id')
ORDER BY column_name;
