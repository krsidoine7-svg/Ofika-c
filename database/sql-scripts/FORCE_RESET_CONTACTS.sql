-- ATTENTION : Ceci effacera toutes les données de test de la table contacts !
DROP TABLE IF EXISTS captured_contacts CASCADE;

-- Création de la table pour stocker les contacts capturés (Leads)
CREATE TABLE captured_contacts (
...
CREATE INDEX idx_captured_contacts_profile_id ON captured_contacts(profile_id);
CREATE INDEX idx_captured_contacts_created_at ON captured_contacts(created_at);
