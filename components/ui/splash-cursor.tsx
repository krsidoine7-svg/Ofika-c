  ...
  useEffect(() => {
    ...
    function getWebGLContext(canvas: HTMLCanvasElement): WebGLContext {
      ...
      if (isWebGL2) {
        ...
        formatRG = getSupportedFormat(gl2, gl2.RG16F, gl2.RG, halfFloatTexType || 0);
        formatR = getSupportedFormat(gl2, gl2.R16F, gl2.RED, halfFloatTexType || 0);
      } else {
        const gl1 = gl as WebGLRenderingContext;
        formatRGBA = getSupportedFormat(gl1, gl1.RGBA, gl1.RGBA, halfFloatTexType || 0);
        formatRG = getSupportedFormat(gl1, gl1.RGBA, gl1.RGBA, halfFloatTexType || 0);
        formatR = getSupportedFormat(gl1, gl1.RGBA, gl1.RGBA, halfFloatTexType || 0);
      }
    ...
    function clickSplat(pointer: Pointer): void {
      ...
      color[0] *= 6.0; // Réduit l'intensité des explosions de couleur
      color[1] *= 6.0;
      color[2] *= 6.0;
      const dx = 8 * (Math.random() - 0.5); // Réduit la force des explosions
    ...
    function updatePointerMoveData(pointer: Pointer, posX: number, posY: number, color: [number, number, number]): void {
      pointer.prevTexcoordX = pointer.texcoordX;
      pointer.prevTexcoordY = pointer.texcoordY;
      pointer.texcoordX = posX / canvas!.width;
      pointer.texcoordY = 1.0 - posY / canvas!.height;
    ...
    function generateColor(): [number, number, number] {
      // Génère des couleurs pastel plus douces
      const c = HSVtoRGB(Math.random(), 0.4, 0.9); // Saturation très réduite et luminosité élevée pour des pastels
      c.r *= 0.05; // Intensité très réduite pour des couleurs très légères
      c.g *= 0.05;
      c.b *= 0.05;
      return [c.r, c.g, c.b];
    }
